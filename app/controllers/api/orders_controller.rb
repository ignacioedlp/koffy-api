class Api::OrdersController < Api::ApiController
  ...
  def index
    ...
    # Usar el scope propio (solo mis órdenes)
    @orders = OrderPolicy::Scope.new(current_user, Order).resolve
  ...
  def create
    ...
    if @order.save
      ...
      # Eliminar los elementos del carrito asociados a los items del pedido
      ordered_variant_ids = @order.order_items.pluck(:coffee_variant_id)
      if current_user.cart
        current_user.cart.cart_items.where(coffee_variant_id: ordered_variant_ids).destroy_all
      end
