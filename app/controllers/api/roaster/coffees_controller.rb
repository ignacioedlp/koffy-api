class Api::Roaster::CoffeesController < Api::Roaster::RoasterController
  ...
  def index
    authorize @roaster, :view_coffees?
    ...
    render json: {
      ...
      meta: {
        ...
        filters: Rails.cache.fetch("coffees_index_filters_#{@roaster.id}", expires_in: 1.hour) do
          {
            categories: @roaster.categories.pluck(:id, :name).map { |id, name| { id: id, name: name } },
            origin_countries: @roaster.coffees.distinct.pluck(:origin_country).compact,
            roast_levels: @roaster.coffees.distinct.pluck(:roast_level).compact,
            process_methods: @roaster.coffees.distinct.pluck(:process_method).compact
          }
        end
      }
    }, status: :ok
  ...
  def create
    ...
    authorize @roaster, :edit_content?
  ...
  def set_coffee
    if @roaster
      @coffee = @roaster.coffees.find_by!(slug: params[:slug])
    else
